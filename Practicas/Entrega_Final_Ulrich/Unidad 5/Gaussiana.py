...
class Gaussian(object):
    ...
    # N > 0
    def __gt__(self, threshold) -> bool:
    ...
    # N >= 0
    def __ge__(self, threshold) -> bool:
        return self.__gt__(threshold)

    # N + M
    def __add__(self, M) -> "Gaussian":
        _mu = self.mu + M.mu
        _sigma = math.sqrt(self.sigma**2 + M.sigma**2)
        return Gaussian(_mu, _sigma)

    # N - M
    def __sub__(self, M) -> "Gaussian":
        _mu = self.mu - M.mu
        _sigma = math.sqrt(self.sigma**2 + M.sigma**2)
        return Gaussian(_mu, _sigma)

    # N * M
    def __mul__(self, M) -> "Gaussian":
        if M.pi == 0:
            return self
        if self.pi == 0:
            return M
    ...
    # N / M
    def __truediv__(self, M) -> "Gaussian":
        _pi = self.pi - M.pi
        _sigma = _pi**-(1/2)
        _mu = (self.tau - M.tau)/_pi
        return Gaussian(_mu, _sigma)
    ...
    # Delta
    def delta(self, M) -> tuple[float, float]:
    ...
    def isapprox(self, M, tol=1e-4):
        return (abs(self.mu - M.mu) < tol
                and abs(self.sigma - M.sigma) < tol)
